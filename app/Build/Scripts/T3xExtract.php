...
function consoleWrite($args)
{
    global $QUIET;
    if ('false' == "$QUIET") {
        file_put_contents('php://stderr', $args);
    }
    return 0;
}
...
foreach ($argv as $_option) {
    ...
    if ($_option == '--verbose') {
        $VERBOSE = 'true';
    } elseif ($_option == '--quiet') {
        $QUIET = 'true';
    } elseif ($_option == '--force') {
        $FORCE = 'true';
    } elseif ($_option == '--update') {
        runSelfUpdate();
    } elseif ($_option == '--update-check') {
        $returnValue = updateCheck();
        exit($returnValue);
    } elseif (0 === strpos($_option, '--base=')) {
        $BASE = substr($_option, strpos($_option, '=') + 1);
    } elseif ($_option == '--export-config') {
        exportConfig();
        exit(0);
    } elseif ($_option == '--extract-config') {
        extractConfig();
        exit(0);
    } elseif (0 === strpos($_option, '--extension=')) {
        $EXTENSION = substr($_option, strpos($_option, '=') + 1);
    } elseif (0 === strpos($_option, '--force-version=')) {
        $FORCE_VERSION = substr($_option, strpos($_option, '=') + 1);
    } elseif ($_option == '--dump') {
        $DUMP    = 'true';
        $EXTRACT = 'false';
    } elseif (0 === strpos($_option, '--string-limit=')) {
        $STRING_LIMIT = substr($_option, strpos($_option, '=') + 1);
    } elseif ($_option == '--extract') {
        $EXTRACT = 'true';
    } elseif (0 === strpos($_option, '--output-dir=')) {
        $OUTPUTDIR = substr($_option, strpos($_option, '=') + 1);
    } elseif ($_option == '--output-file') {
